...
const TypingBox = () => {
  ...
  /* resetting all the classes on click of time */
...
//   const { testTime } = useTestMode(); // getting the access to value object inside context
//   const [countDown, setCountDown] = useState(testTime); // state for couter
//   const [testStart, setTestStart] = useState(false); // for checking test started or not
//   const [testEnd, setTestEnd] = useState(false); // for checking test ended or not
//   const [intervalId, setIntervalId] = useState(null); // to set the intervals in timer
...
//   const [missedChars, setMissedChars] = useState(0); // counting missed chars
//   const [extraChars, setExtraChars] = useState(0); // counting extra chars
//   const [correctWords, setCorrectWords] = useState(0); // counting correct words
//   const [graphData, setGraphData] = useState([]);
...
//     // handle space logic
//     if (e.keyCode === 32) {
//       /* checking if user typed the whole word correctly */
//       let correctCharsInWord =
//         wordsSpanRef[currWordIndex].current.querySelectorAll(".correct");
//       if (correctCharsInWord.length === allCurrChars.length) {
//         console.log("helloo");
...
//     // Handle backspace logic
//     if (e.keyCode === 8) {
//       // avoiding user from going back to the previous word
//       // only the current word is allowed
//       if (currCharIndex !== 0) {
//         // for the last index of word --> backspace
//         if (allCurrChars.length === currCharIndex) {
//           // for the extra word removal -->  backspace
...
//   /* removes the word from the list of words (407)*/
//   // const removeWord = () => {
//   //   setWordsArray((prevWordsArray) => {
//   //     const updatedWordsArray = [...prevWordsArray];
//   //     updatedWordsArray.splice(0, 1);
//   //     return updatedWordsArray;
//   //   });
//   // };

//   /* focus the input field when reloaded */
//   const focusInput = () => {
//     inputRef.current.focus();
//   };
...
//   /* when the site loads, input field should be focused */
//   useEffect(() => {
//     focusInput();
//     wordsSpanRef[0].current.childNodes[0].className =
//       "active"; /* for blinking purpose when site reloads */
...
//         />
//       ) : (
//         /* we are creating single spans for single characters
//         in single word each character should be pushed into a new span
//         */
//         <div className="type-box" onClick={focusInput}>
//           <div className="words">
//             {wordsArray.map((word, index) => (
